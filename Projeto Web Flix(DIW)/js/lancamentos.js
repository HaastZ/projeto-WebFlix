...
const mostraFilme = (data) =>{
    ...
    let dadosFilme = JSON.parse (data.target.response)
    localStorage.setItem('db_filmes', data.target.response)

    //da pagina de destaques
    let dadosHTM = '';
    for(let i = 0; i < dadosFilme.results.length; i++){
          let filme = dadosFilme.results[i];
          dadosHTM += `
          <div class="card col-2" style="background-color:black">
      ...
      document.getElementById('lancamentoLista').innerHTML = dadosHTM
}

const mostraErros = (data) => {
     alert('Erro na requisição')
}

const init = ()=> {
    let xhr = new XMLHttpRequest ();
    let url = "https://api.themoviedb.org/3/movie/top_rated?api_key=2b4761a31849f8d427d59cedc8f92c80&language=pt-BR"
    xhr.onload = mostraFilme;
    xhr.onerror = mostraErros;
